...
Date : 2019.07.30
Title: Simple TKinter Class 03-05
Simple Tkinter class with Picture Label, button, functions and methods.
A Picture label showing a .jpg picture, with at width size of 500 pixels. The label picture has 5 buttons assigned that can change the picture
'''
...
class my_gui:
	def __init__(self, master):
		...
		self.image_resized = self.image.resize((self.image_width, int(image_size[1]*self.image_width/image_size[0])))
		self.photo = ImageTk.PhotoImage(self.image_resized)
		self.my_label1 = Label(master, image=self.photo, width=self.image_width)
