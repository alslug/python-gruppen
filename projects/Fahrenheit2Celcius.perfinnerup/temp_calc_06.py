...
def konverter():
    try:
        if my_listbox1.curselection()[0] == 0:  #Celcius 2 Fahrenheit
            logging.debug('change from ceicius to fahrenheit')
            my_entry2_str.set(float(my_entry1_str.get()) * 1.8 + 32)
        elif my_listbox1.curselection()[0] == 1:  #Fahrenheit 2 Celcius
            logging.debug('change from fahrenheit to ceicius')
...
my_listbox1.bind("<1>", listbox1_change)
